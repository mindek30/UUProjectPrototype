...
uint8_t APPL_USB_Rx_SerialBuffer[100];
/* USER CODE END PV */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  APPL_DMA_LTE_Init();
  EKM_Buffer_Setup(&APPL_USB_Rx,100,APPL_USB_Rx_SerialBuffer);
  ...
  while (1)
  {
    ...
    if(APPL_USB_Rx.write>0){
      APPL_DMA_LTE_printf("%s",APPL_USB_Rx.buffer);
